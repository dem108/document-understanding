...
def analyze_general_documents(path_to_documents, f_desc, f_kvp_csv, f_entity_csv, f_item_csv, f_table_csv, f_table2_csv):
    ...
    def write_table2_csv(s):
        print("{}".format(s), file=f_table2_csv)
    ...
    # show_handwritten = False
    show_keyvaluepair = True
    # show_keyvaluepair = False
    show_entity = True
    # show_entity = False
    show_item = True
    # show_item = False
    ...
    if show_table:
        ...
        for table_idx, table in enumerate(result.tables):
            ...
            write_table2_csv("")
            write_table2_csv(",".join(["document_name", "page_number", "table_idx", "table_row_count", "table_column_count", "table_bb"]))

            write_table2_csv(",".join([path_to_documents, 
            ...
            for cell in table.cells:
                ...
                df = df.append([[cell.row_index, cell.column_index, cell.content]])

            df2 = df.pivot(index=0, columns=1, values=2)
            write_table2_csv(df2.to_csv(line_terminator="\n"))
            # for line in df2.itertuples():
            #     write_table2_csv(str(list(line)).replace('[','').replace(']','').replace("'",""))

            # write_table2_csv(",".join([
            #     str(cell.row_index),
            #     str(cell.column_index),
            #     format_content(cell.content),
            #     format_bounding_box(cell_region_bb)]))
...
if __name__ == "__main__":
    ...
    with open(os.path.join(script_path, "..", "results", "gen_doc.txt"), "a", encoding='utf-8-sig') as f_desc, \
        open(os.path.join(script_path, "..", "results", "gen_doc_kvp.csv"), "a", encoding='utf-8-sig') as f_kvp_csv, \
        open(os.path.join(script_path, "..", "results", "gen_doc_entity.csv"), "a", encoding='utf-8-sig') as f_entity_csv, \
        open(os.path.join(script_path, "..", "results", "gen_doc_item.csv"), "a", encoding='utf-8-sig') as f_item_csv, \
        open(os.path.join(script_path, "..", "results", "gen_doc_table.csv"), "a", encoding='utf-8-sig') as f_table_csv, \
        open(os.path.join(script_path, "..", "results", "gen_doc_table2.csv"), "a", encoding='utf-8-sig') as f_table2_csv:
        ...
        for path_to_file in str_path_to_files:
            ...
            analyze_general_documents(os.path.normpath(path_to_file), f_desc, f_kvp_csv, f_entity_csv, f_item_csv, f_table_csv, f_table2_csv)
